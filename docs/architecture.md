...
| `short_links` | 短链接表 |
| `domains` | 域名表 |
| `domain_group_domains` | 用户组-域名关联表 |
| `templates` | 跳转模板表 |
| `access_logs` | 访问日志表 |
| `api_keys` | API 密钥表 |
| `audit_logs` | 审计日志表 |
...
| 角色 | 说明 | 用户组 |
|------|------|--------|
| `admin` | 管理员 | 管理员组（虚拟，不存储于数据库） |
| `user` | 普通用户 | 可分配到具体用户组 |

**管理员特性**：
- 管理员属于特殊的"管理员组"（虚拟组，不在数据库中）
- 管理员不能被分配到任何用户组
- 管理员可以访问所有启用的域名
- 管理员不能禁用自己
- 前端在用户列表中显示"管理员组"标识
...
**用户组域名权限**：
- 普通用户只能使用其用户组被授权的域名创建短链接
- 用户组域名关联通过 `domain_group_domains` 表管理
- 如果用户没有用户组，返回空域名列表
- 普通用户访问 `/api/user/domains` 接口只能看到其用户组授权的域名
