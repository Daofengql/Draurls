...
type ShortLink struct {
	ID          uint            `gorm:"primarykey"`
	Code        string          `gorm:"uniqueIndex;size:20;not null;comment:短码"`
	URL         string          `gorm:"type:text;not null;comment:目标URL"`
	UserID      uint            `gorm:"index;not null;comment:创建用户ID"`
	...
	Status      LinkStatus      `gorm:"size:20;not null;default:'active';comment:状态"`
	CreatedAt   time.Time
	UpdatedAt   time.Time
	DeletedAt   *time.Time      `gorm:"index"`
}
...
type APIKey struct {
	...
	Status      APIKeyStatus `gorm:"size:20;not null;default:'active';comment:状态"`
	CreatedAt   time.Time
	UpdatedAt   time.Time
	DeletedAt   *time.Time `gorm:"index"`
}
...
type AccessLog struct {
	ID        uint      `gorm:"primarykey"`
	LinkID    uint      `gorm:"index;not null;comment:链接ID"`
